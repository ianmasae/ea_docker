//+------------------------------------------------------------------+
//| BridgeService.mq5 — TCP Socket Bridge Service for Python API      |
//|                                                                    |
//| MQL5 Service that runs independently without chart attachment.     |
//| Connects to the Python FastAPI server via TCP socket.              |
//| Once started, auto-runs on all subsequent MT5 startups.            |
//| Protocol: 4-byte big-endian length prefix + JSON payload.          |
//+------------------------------------------------------------------+
#property service
#property copyright "MT5 Docker Bridge"
#property version   "1.00"
#property description "TCP bridge service for Python Trading API"
#property strict
...
input int    InpPollMs     = 100;           // Poll interval (ms)
...
void OnStart()
{
   Print("BridgeService: Started. Connecting to ", InpHost, ":", InpPort);
...
void Connect()
{
   ...
   {
      Print("BridgeService: SocketCreate failed: ", GetLastError());
      return;
   }
   ...
   if(!SocketConnect(g_socket, InpHost, InpPort, 3000))
   {
      ...
      Print("BridgeService: Connect failed to ", InpHost, ":", InpPort, " error=", err);
   ...
   ArrayResize(g_recvBuf, 0);
   g_recvBufLen = 0;
   Print("BridgeService: Connected to ", InpHost, ":", InpPort);
}
...
void ReadAndProcess()
{
   ...
   if(read <= 0)
   {
      Print("BridgeService: Socket read error, disconnecting");
      Disconnect();
      return;
   }
   ...
   while(g_recvBufLen >= 4)
   {
      ...
      if(msgLen > 1000000)
      {
         Print("BridgeService: Invalid message length: ", msgLen);
         Disconnect();
         return;
      }
...
bool SendResponse(string json)
{
   ...
   uchar payload[];
   int payloadLen = StringToCharArray(json, payload, 0, -1, CP_UTF8) - 1;
   ...
   {
      Print("BridgeService: Send failed (sent=", sent, " expected=", 4 + payloadLen, ")");
      Disconnect();
      return false;
   }
   return true;
